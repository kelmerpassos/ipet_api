...
def test_token_refresh_return_code(authentication_refresh, client):
    assert client.post("/auth/token/refresh", headers=authentication_refresh, follow_redirects=True).status_code == OK

def test_token_update(authentication_refresh, client):
...
def test_token_invalidation_return_code(authentication, client):
    assert client.post("/auth/token/invalidate", headers=authentication, follow_redirects=True).status_code == OK

def test_token_invalidation(authentication, client):
