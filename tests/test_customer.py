...
def test_customer_listing_return_code(authentication, client: FlaskClient):
    assert (
        client.get(
            "/customer", headers=authentication, follow_redirects=True
        ).status_code
        == OK
    )
...
def test_customer_return_code_by_id(authentication, client: FlaskClient):
    assert (
        client.get(
            "/customer/1", headers=authentication, follow_redirects=True
        ).status_code
        == OK
    )
...
def test_return_code_create_customer(
    authentication, client: FlaskClient, customer_json
):
    assert (
        client.post(
            "/customer",
...
def test_create_customer(authentication, client: FlaskClient, customer_json):
    client.post(
        "/customer", json=customer_json, headers=authentication, follow_redirects=True
    )
    assert Customer.query.filter(Customer.cpf == 11111111111).first() is not None


def test_authentication_obligation_in_create_customer(
    client: FlaskClient, customer_json
):
    assert (
        client.post("/customer", json=customer_json, follow_redirects=True).status_code
        == UNAUTHORIZED
    )


def test_return_code_update_customer(
    authentication, client: FlaskClient, customer_json
):
    assert (
        client.put(
...
def test_update_customer(authentication, client: FlaskClient, customer_json):
    client.put(
        "/customer/1", json=customer_json, headers=authentication, follow_redirects=True
    )
    assert Customer.query.filter(Customer.cpf == 11111111111).first() is not None


def test_authentication_obligation_in_update_customer(
    client: FlaskClient, customer_json
):
    assert (
        client.put("/customer/1", json=customer_json, follow_redirects=True).status_code
        == UNAUTHORIZED
    )


def test_return_code_partially_update_customer(
    authentication, client: FlaskClient, customer_json
):
...
def test_partially_update_customer(authentication, client: FlaskClient, customer_json):
    customer_json.pop("cpf")
    retorno = client.patch(
        "/customer/1", json=customer_json, headers=authentication, follow_redirects=True
    )
    assert Customer.query.filter(Customer.full_name == "testing").first() is not None


def test_authentication_obligation_in_partially_update_customer(
    client: FlaskClient, customer_json
):
    assert (
        client.patch(
            "/customer/1", json=customer_json, follow_redirects=True
        ).status_code
        == UNAUTHORIZED
    )
...
def test_authentication_obligation_in_delete_customer(client: FlaskClient):
    assert (
        client.delete("/customer/1", follow_redirects=True).status_code == UNAUTHORIZED
    )
...
def test_return_code_customer_products(authentication, client: FlaskClient):
    assert (
        client.get(
            "/customer/1/product", headers=authentication, follow_redirects=True
        ).status_code
        == OK
    )
...
def test_authentication_obligation_in_customer_products(client: FlaskClient):
    assert (
        client.get("/customer/1/product", follow_redirects=True).status_code
        == UNAUTHORIZED
    )


def test_return_code_product_and_customer_information(
    authentication, client: FlaskClient
):
    assert (
        client.get(
            "/customer/1/product/1", headers=authentication, follow_redirects=True
        ).status_code
        == OK
    )
...
def test_product_and_customer_information(authentication, client: FlaskClient):
    ...
    assert (
        ...
        and data["currentStatus"] in STATUS_CHOICE
    )


def test_authentication_obligation_in_product_and_customer_information(
    client: FlaskClient,
):
    assert (
        client.get("/customer/1/product/1", follow_redirects=True).status_code
        == UNAUTHORIZED
    )


@mark.parametrize("status", STATUS_CHOICE)
def test_return_code_update_customer_product_status(
    authentication, client: FlaskClient, status
):
    assert (
        client.patch(
...
def test_update_customer_product_status(authentication, client: FlaskClient):
    client.patch(
        "/customer/1/product/1",
        headers=authentication,
        json={"currentStatus": "BLOCKED"},
        follow_redirects=True,
    ).get_json()
    assc = AssocProductCustomer.query.filter(
        AssocProductCustomer.customer_id == 1, AssocProductCustomer.product_id == 1
    ).first()
    assert assc.current_status == "BLOCKED"


def test_authentication_obligation_in_update_customer_product_status(
    client: FlaskClient,
):
    assert (
        client.patch(
            "/customer/1/product/1",
            json={"currentStatus": "ACTIVE"},
            follow_redirects=True,
        ).status_code
        == UNAUTHORIZED
    )
